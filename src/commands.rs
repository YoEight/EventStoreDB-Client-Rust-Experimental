...
fn build_request_metadata<Opts>(
    ...
    Opts: crate::options::Options,
{
    ...
    let credentials = options
        .get_credentials()
        .or_else(|| settings.default_authenticated_user().clone());
    ...
    if options.is_requires_leader_set() {
    ...
    Opts: crate::options::Options,
{
    ...
    *req.metadata_mut() = build_request_metadata(settings, options);
    ...
    if let Some(duration) = options.get_deadline() {
...
pub fn subscribe_to_stream<S: AsRef<str>>(
    ...
    let metadata = build_request_metadata(connection.connection_settings(), options);
